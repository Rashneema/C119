...
def get_stem_words(words, ignore_words):
    ...
    for word in words:
        if word not in ignore_words:
            w = stemmer.stem(word.lower()) #to change the word into its stem or root word
            stem_words.append(w)  
    return stem_words
...
#print("tag words==>",word_tags_list) #index 0 because we are appending a tuple
#print("Final tag==>",classes)   
...
for word_tags in word_tags_list:
        ...
        bag_of_words = []       
        pattern_words = word_tags[0] # index 0 as items are inside a tuple 
...
def preprocess_train_data(training_data):
    ...
    train_x = list(training_data[:,0]) #BOW
    train_y = list(training_data[:,1]) #tag
